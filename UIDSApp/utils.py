...
def IntraIDTimeGapNorm(value):
    """Normalize 'Intra_ID_Time_Gap' into discrete bins."""
    if value is None:
        return -1
...
def TimeDeltaTimeGapNorm(value):
    """Normalize 'Time_Delta' into discrete bins."""
    if value is None:
        return -1
    if 0 <= value <= 0.05: return 0
    elif value <= 0.1: return 1
...
def SegmentForValidation(data_directory, filename, time_gap):
    """
    ...
    while window_start <= max_time:
        ...
        seg = df_subset[(df_subset['Time_Offset'] >= window_start) &
...
def SegmentForInference(data_directory, filename, time_gap):
    """
...
def ChunkSizeMatch(data_input, chunk_size=265, max_time_ms=3000):
    """
    Dynamically estimate average segment time from CAN CSV files or a DataFrame.
    - If given a DataFrame: process directly.
    - If given a list of file paths: process the first valid CSV.

    Args:
        data_input (list[str] | pd.DataFrame): Either a DataFrame or list of CSV paths.
        chunk_size (int): Number of messages per chunk.
        max_time_ms (int): Maximum allowed chunk duration in ms.

    Returns:
        float: Average time (in seconds) per valid chunk (for model segmentation).
    """
    # --- 1. Handle input type ---
    if isinstance(data_input, list):
        # Try loading the first valid CSV
        if len(data_input) == 0:
    ...
    # --- 2. Ensure 'Time_Offset' column exists ---
    ...
    for start_idx in range(0, total_msgs, chunk_size):
        ...
        if len(segment_df) >= int(chunk_size * 0.8):  # at least 80% full
