...
# Partially Observable Environment for Time Series Classification
class POMDP_TimeSeriesEnv:
    def __init__(self, data, labels, observation_prob=0.8):
        self.data = data
        self.labels = labels
        self.current_index = 0
        self.observation_prob = observation_prob  # Probability of observing the true state
    
    def reset(self):
        self.current_index = 0
        return self.get_observation(self.data[self.current_index])
    ...
    def step(self, action):
        ...
        next_state = self.get_observation(self.data[self.current_index]) if not done else None
    ...
    def get_observation(self, state):
        if random.random() < self.observation_prob:
            ...
            return torch.randn_like(state)  # Noisy state
...
class DQN_GRU(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.gru(x.unsqueeze(0))  # Add batch dimension
        x = self.fc(x[:, -1, :])  # Take last time step output
        return x

# GRU-based Smaller Deep Q-Network (Student Model)
class DQN_GRU_Student(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim=32):  # smaller hidden dim
...
# Distillation Loss Function (Soft & Hard Targets)
...
# Training the Student Network via Distillation
def distill_student(student, teacher, env, epochs=10, batch_size=32, lr=0.001):
    optimizer = optim.Adam(student.parameters(), lr=lr)
    buffer = ReplayBuffer()
    
    # Populate buffer with teacher transitions
    for _ in range(100):  # 100 episodes to fill buffer
        state = env.reset()
...
# Training the Teacher Network (DQN_GRU)
...
env = POMDP_TimeSeriesEnv(data, labels)

# Train Teacher Network
trained_q_network, model_path = train_dqn(env)

# Load Teacher Model
teacher = DQN_GRU(input_dim=env.data.shape[1], output_dim=2)
teacher.load_state_dict(torch.load(model_path))
teacher.eval()

# Train Student Network via Distillation
student = DQN_GRU_Student(input_dim=env.data.shape[1], output_dim=2)
